...
export type UncertainReason =
  ...
  /** حروف مقطَّعة — لا يقدر مزوّدُ كلامٍ عامّ على تفريغها أصلًا. */
  | 'MUQATTAAT';
...
function applyConservatism(
  ...
  return entries.map((e, i) => {
    ...
    /**
     * ٤) حروف مقطَّعة ⇒ لا حكم عليها إطلاقًا.
     *
     * ⚠️ قِيس هذا لا خُمِّن: «الٓمٓ» في أول البقرة رجعت من المزوّد
     * «اليك». فهو لا يعرف أسماء الحروف المتتابعة — ليست في لغته —
     * فيلفّق أقرب كلمة عربية مهما أحسنت القارئة.
     *
     * وعجزُ الآلة ليس خطأً من الطالبة. وهذه كلمةٌ واحدة في كلٍّ من
     * ٢٩ سورة، فالتنازل عن الحكم فيها ثمنٌ زهيد أمام اتهامٍ باطلٍ
     * يقع في أول ما تفتحه.
     */
    for (const w of e.expected) {
      if (isMuqattaat(w.uthmani)) return uncertain('MUQATTAAT');
    }

    /**
     * ٥) استبدالٌ بكلمةٍ من المقطع نفسه ⇒ قد يكون صدى المزوّد.
     *
     * ⚠️ قِيس أيضًا: «يُنفِقُونَ» رجعت «يوقنون» — وهي آخر كلمة في
     * الآية التالية مباشرة. فالمزوّد خلط بين متقاربين وسحب الثانية
     * من النص الذي زوّدناه به.
     *
     * وهي علّة صدى الزيادة نفسها، تقع في الاستبدال. والمتشابهات في
     * القرآن كثيرة، فخلطُ الآلة بينها أرجح من نسيان الحافظة لها.
     */
    if (
      e.kind === 'SUBSTITUTION' &&
      e.heard.length === 1 &&
      inPassage[e.heard[0].norm]
    )
      return uncertain('ECHO_OF_PASSAGE');

    // ٦) استبدال بكلمة قريبة ⇒ الأرجح خطأ سماع لا خطأ حفظ
    //
    ...
    // ٧) كلمة زائدة موجودة في المقطع ⇒ قد تكون صدى المزوّد لا صوتها
    //
    ...
    // ٨) حذفٌ في طرفي المقطع ⇒ الأرجح أن التسجيل قُصّ لا أنها نسيت
    //
    ...
    if (e.kind === 'OMISSION' || e.kind === 'SKIP') {
      ...
      /**
       * ٩) حذفٌ قصير يجاوره نصٌّ متكرّر ⇒ لا يُعرف أيّ نسخة سقطت.
       *
