...
export default function MyAgendaLockedPage() {
  return (
    <main className="min-h-dvh flex flex-col items-center justify-center px-5 py-10 text-center">
      ...
      <div className="card-3d w-full max-w-md p-10 mt-6 animate-float-in" style={{ animationDelay: '0.1s' }}>
        <span aria-hidden="true" className="text-5xl">
          🌿
        </span>
        <h1 className="mt-4 text-xl font-extrabold text-ink">أجندتي — خاص بالمشتركين</h1>
        <p className="mt-3 text-ink/60 leading-relaxed">
          أجندة مهنية بسيطة وأنيقة للمعلّم والمعلّمة: نظّم مهامك اليومية والأسبوعية والشهرية،
          وثّق إنجازاتك بالصور، وأنشئ ملف إنجاز جاهز للتسليم — كل شيء يُحفظ سحابيًا ويُستأنف بين أجهزتك.
          <br />
          فعّل اشتراكك للبدء.
        </p>
        ...
        <div className="mt-6 rounded-2xl border border-sage/25 bg-sage-light/40 p-4 text-right">
          ...
          <ul className="text-sm text-ink/70 leading-relaxed space-y-1">
            <li>✅ مهام يومية وأسبوعية وشهرية مع تكرار وأولويات</li>
            <li>📅 تقويم شهري ومواعيد قادمة تلقائية</li>
            <li>⭐ توثيق الإنجازات بالصور وملف إنجاز قابل للطباعة</li>
            <li>☁️ حفظ تلقائي واستئناف بين الأجهزة</li>
          </ul>
        </div>
        ...
        <Link href="/" className="mt-3 block text-ink/55 hover:text-ink font-bold text-sm transition-colors">
          العودة للرئيسية
        </Link>
      </div>
    </main>
