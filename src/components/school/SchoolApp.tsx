...
export default function SchoolApp({
  ...
  const [loading, setLoading] = useState(Boolean(initialSchoolId));
  const [view, setView] = useState<'dash' | 'structure'>('dash');
  const [toast, setToast] = useState('');

  const canManage = isAdmin || memberRole === 'admin' || memberRole === 'principal';
  ...
  const createSchool = async (name: string, year: string, term: string, days: number[]) => {
    ...
    if (error || !data) {
      showToast(/forbidden/i.test(error?.message || '') ? 'اشتراكك لا يشمل إدارة المدرسة' : 'تعذّر إنشاء المدرسة');
      return;
    }
    setSchoolId(data as string);
  };
  ...
  if (!schoolId) {
    return (
      <Shell firstName={firstName} title="إدارة المدرسة">
        {canCreate ? (
          <SetupWizard onCreate={createSchool} />
        ) : (
          <div className="card-3d bg-white rounded-2xl p-8 text-center text-ink/70">لا توجد مدرسة مرتبطة بحسابك بعد.</div>
        )}
