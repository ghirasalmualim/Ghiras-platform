...
console.log("═══ ٥ · المعرفة: أرقام معتمدة فقط ═══");
{
  ...
  check("أسعار المواد المعتمدة: ٢٠ للمادة و٥٠ للمرحلة — كلاهما ٦ أشهر",
    k.includes("٢٠ د.ك لمدة ٦ أشهر") && k.includes("٥٠ د.ك لمدة ٦ أشهر") && k.includes("[MATERIALS"));
  check("سجل الحضور بسعره المنشور والإضافي ٤", k.includes("٨ د.ك لستة أشهر") && k.includes("٤ د.ك"));
  check("ما لا سعر له معلن → الإدارة", k.includes("بلا سعر معلن") && !/بلا سعر معلن[^\n]*سجل الدرجات/.test(k));
  check("دفتر الدرجات ٨ د.ك / ٦ أشهر", k.includes("[GRADEBOOK") && /GRADEBOOK[^\[]*٨ د\.ك لمدة ٦ أشهر/.test(k));
  check("المغامرة ٨/٦ أشهر ورئيس القسم ٣٠/سنة",
    /ADVENTURE[^\[]*٨ د\.ك لمدة ٦ أشهر/.test(k) && /HEAD_RECORDS[^\[]*٣٠ د\.ك لمدة سنة/.test(k));
  check("لم يبقَ بلا سعر إلا الورش", /بلا سعر معلن حاليًا \(الورش التعليمية\)/.test(k));
  check("القرآن مجاني والتسميع بدخول بلا اشتراك",
    k.includes("[QURAN") && k.includes("مجاني بالكامل") && k.includes("تسجيل دخول فقط") && !/QURAN[^\[]*د\.ك/.test(k));
