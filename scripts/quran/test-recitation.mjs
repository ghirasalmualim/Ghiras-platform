...
{
  const exp = buildExpected(ayahs(112, 1, 4));
  const h1 = nextHint(exp, 4, 0);
  ok(h1.kind === "ENCOURAGE", "الدرجة الأولى تشجيع لا جواب");
  const h2 = nextHint(exp, 4, 1);
  ok(h2.kind === "PLAY" && h2.surah === 112, "الثانية تشغّل الموضع من القارئ");
  const h3 = nextHint(exp, 4, 2);
  ok(h3.kind === "REVEAL" && h3.words.length <= 2,
     `الثالثة تكشف كلمتين لا أكثر (${h3.words.length})`);
  ok(h3.words[0] === exp[4].uthmani, "⚠️ والكلمة من النص المرجعي حرفًا بحرف");
  ok(nextHint(exp, 4, 3) === null, "⚠️ ولا درجة رابعة — لا يُعرض المقطع كله أبدًا");
}
