...
/**
 * ⚠️ **يُحسب من المحرّك لا بفحصٍ نصّيّ.**
 *
 * كُتب أوّلًا بفحصٍ خام (`/ۢ$/`) يلتقط العلامة إن كانت آخر محرفٍ
 * حرفيًّا، فأعطى ١٤ — بينما وصفُ القاعدة يقول ٨٤ (وهو أصلًا فرقُ
 * تعريفٍ مع مصدرٍ خارجيّ لا امتناعًا منّا). ورقمان متناقضان في ورقةٍ
 * واحدة يُسقطان ثقة المختصّ في الباقي كلّه.
 *
 * فصار يُشتقّ ممّا فعله المحرّك فعلًا: كلُّ علامة إقلابٍ لم يشملها
 * حكم. فلا يفترق العددُ عن الواقع مهما تغيّر أحدهما.
 */
const crossAyah = [];
for (const v of AYAH) {
  const iq = annotateAyah(v.text, v.surah, v.ayah).filter((x) => x.rule === "iqlab");
  for (let i = 0; i < v.text.length; i++) {
    if (v.text[i] !== "ۢ") continue;
    if (iq.some((x) => x.start <= i && i < x.end)) continue;
    const nx = AYAH.find((u) => u.surah === v.surah && u.ayah === v.ayah + 1);
    crossAyah.push({ ...v, next: nx ? nx.text.slice(0, 22) : "(آخر السورة)" });
    break;
  }
}
...
const iqTotal = [...AYAH].reduce(
  (n, v) => n + [...v.text].filter((c) => c === "ۢ").length, 0);
const iqJudged = (all.get("iqlab") ?? []).length;
md += `في المصحف **${N(iqTotal)}** علامةَ إقلاب. حكمنا بـ**${N(iqJudged)}**،\n`;
md += `وامتنعنا عن **${N(iqTotal - iqJudged)}** — وهي **كلُّها** في أواخر الآيات بلا استثناء،\n`;
md += `تنتهي الآية بتنوينٍ عليه العلامة وتبدأ التاليةُ بباء.\n\n`;
