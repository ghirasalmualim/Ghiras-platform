...
// ══════════════ ١٩ز) كلامٌ ليس تلاوة — من واقعة حقيقية ══════════════
//
// ⚠️ فُتح التسجيل والناس يتحدّثون ولم تُقرأ آية، فقيل للقارئة
// «أتقنتِ ستة مواضع». وسببُه أن حارس الصلاحية كان يعدّ **ما سُمع**
// لا **ما طابق**: الحديثُ العادي يُنتج كلمات كثيرة فيمرّ، ثم تصادف
// كلماتٌ شائعة مواضعَها فتُحسب إتقانًا.
console.log("\n  ── ١٩ز) كلامٌ ليس تلاوة ──");
{
  const exp = buildExpected(ayahs(2, 1, 5));

  // كلامٌ عادي فيه كلمات شائعة تصادف مواضعها
  const chatter =
    "السلام عليكم شلونكم اليوم الحمد لله على كل حال من الله علينا " +
    "هم يقولون بس ما ادري في شي ثاني ولا لا خلاص يالله";
  const r = run(exp, chatter.split(" "));

  ok(!r.usable, "حديثٌ عادي لا يُحكَم عليه أصلًا");
  ok(r.unusableReason === "NOT_THIS_PASSAGE", "والسبب: ليست تلاوةً لهذا المقطع");
  ok(
    r.summary.matched === 0 && r.summary.confirmedErrors === 0,
    "⚠️ ولا يُقال «أتقنتِ كذا موضعًا» لمن لم تقرأ حرفًا",
    `مطابق ${r.summary.matched} · أخطاء ${r.summary.confirmedErrors}`
  );
  ok(r.weakSpots.length === 0, "ولا يُرسَل شيء إلى المراجعة");

  // ⚠️ ولا يُشلّ الحكم: تلاوةٌ فيها أخطاء كثيرة تبقى صالحة
  const words = perfect(exp);
  const many = words.filter((_, i) => i % 4 !== 0); // يسقط ربعها
  const r2 = run(exp, many);
  ok(
    r2.usable,
    "وتلاوةٌ سقط ربعها تبقى صالحة للحكم — الحارس ليس تعطيلًا",
    `مطابق ${r2.summary.matched} من ${r2.summary.expectedWords}`
  );
}
