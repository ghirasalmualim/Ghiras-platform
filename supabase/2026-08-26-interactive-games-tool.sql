-- ═══════════════════════════════════════════════════════════════
-- «ألعاب غراس التفاعلية» — صلاحية قسمٍ مستقلة (Staging أولًا، ثم الإنتاج بيد صاحبة المنصة)
--
-- وصول قسم الألعاب كان بتسجيل الدخول وحده؛ الآن يصبح استحقاقًا يمنحه
-- ويسحبه الأدمِن كبقية الأدوات. لا يمسّ game_credits (رصيد التوليد
-- منتج آخر تمامًا) ولا المنتجات المستقلة (الساعة/جدول الضرب).
-- الدالة أدناه هي النسخة الحرفية المطبقة على الإنتاج + حالة واحدة جديدة.
-- ═══════════════════════════════════════════════════════════════

alter table public.profiles
  add column if not exists interactive_games_until timestamptz;
...
begin
  ...
  v_col := case p_tool
    ...
    when 'interactive_games' then 'interactive_games_until'
    else null
  end;
